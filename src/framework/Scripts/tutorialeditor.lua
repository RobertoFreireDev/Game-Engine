...
function tutorialeditor:create()
    ...
    function luafunctions:init()
        ...
        add(self.func,{ l = "_ngrid", c = "void NewGrid(int columns, int rows, int size, bool enableUndoRedo = false)"})
        add(self.func,{ l = "_ggrid", c = "string GetGrid()"})
        add(self.func,{ l = "_ggrid64", c = "string GetGridAsBase64(int x, int y, int w, int h)"})
        add(self.func,{ l = "_cgrid", c = "void CopyGrid(int x, int y, int w, int h)"})
        add(self.func,{ l = "_pgrid", c = "void PasteGrid(int x, int y, int w, int h)"})
        add(self.func,{ l = "_mgrid", c = "void MoveGrid(int x, int y, int w, int h, int deltaX, int deltaY)"})
        add(self.func,{ l = "_sgrid", c = "void SetGrid(string grid)"})
        add(self.func,{ l = "_ugrid", c = "void UndoGrid()"})
        add(self.func,{ l = "_rgrid", c = "void RedoGrid()"})
        add(self.func,{ l = "_bgrid", c = "void PaintBucket(int sx, int sy, int x, int y, int w, int h, int colorIndex = -1)"})
        add(self.func,{ l = "_gpixelgrid", c = "int GetPixel(int x, int y)"})
        add(self.func,{ l = "_spixelgrid", c = "void SetPixel(int x, int y, int colorIndex = -1)"})
        ...
        -- Map
        add(self.func,{ t = "-- Map --"})
        add(self.func,{ l = "_lmap", c = "void SetMap(string grid)"})
        add(self.func,{ l = "_cmap", c = "void CreateMap(int columns, int rows, int size)"})
        add(self.func,{ l = "_gmap", c = "string GetMap()"}) 
        add(self.func,{ l = "_smap", c = "void SetTileInMap(int x, int y, int tileIndex = 0)"})
        add(self.func,{ l = "_bmap", c = "void UpdateTileInMap(int mapX, int mapY,int width, int height, int tileIndex = 0)"})
        add(self.func,{ l = "_dmap", c = "void DrawMap(int mapX, int mapY, int x, int y, int width, int height, int colorIndex = -1, int transparency = 10)"})
        add(self.func,{ t = ""})
    end
